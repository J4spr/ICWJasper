    ...
    for i in range(len(pixels)):  # for the height of the image
        ...
        for j in range(len(pixels[i])):  # for the width of the image
            ...
            if dark:  # if the background is dark, the characters will be light, so "██" will represent white. Otherwise "  " will represent white
                if pixels[i][j] > (4 / 5) * 765:
                    row.append("██")
                elif pixels[i][j] > (3 / 5) * 765:
                    row.append("▓▓")
                elif pixels[i][j] > (2 / 5) * 765:
                    row.append("▒▒")
                elif pixels[i][j] > (1 / 5) * 765:
                    row.append("░░")
                else:
                    row.append("  ")
            else:
...
def renderVideoInTerminal(frameCount, frameRate, path, res=6, audio=False):
    ...
    for i in range(frameCount):  # loop for the number of frames
        iNum = list(
            str(i + 1))  # the file names follow a pattern of a number increasing, so this just finds the frame we should be calling
        if len(list(str(i + 1))) < 5:  # the number is always 5 digits long, so
            for l in range(
                    5 - len(list(str(i + 1)))):  # we need to add zero's to the start of it in case i < 5 digits long
                iNum.insert(0, '0')

        fPath = path + str(''.join(
            iNum)) + ".png"  # file path/frame we want; the "+str(''.join(iNum))+" in the middle is because that part of the filename counts upwards, as mentioned previously.
        frames.append(convertImageToShades(readBitmapBrightness(fPath, res),
                                           True))  # appends the text version of the frame to the list of frames with 1/6th resolution

        if i == (
                round(
                    frameCount / 10)) * dones:  # if the number of frames has passed a multiple of 5% of all the frames to render;
            print(("\n" * 200) + "Pre-rendering frames...\n\n" + str(
                round(i / frameCount * 100)) + "% complete...")  # print out the percentage complete
...
def renderVideoInNotepad(frameCount, frameRate, path, res=6,
                         audio=False):  # opens up a notepad window for every frame and then closes it

    try:
        os.makedirs('notepadFrames')
    except OSError:
        pass
    ...
    for i in range(frameCount):  # loop for the number of frames
        ...
        with open("notepadFrames/notepadFrame" + str(''.join(iNum)) + ".txt", 'w',
                  encoding="utf-8") as frame:  # we're using unicode characters, so specifying the encoding as "utf-8" is necessary
            frame.write(str(convertImageToShades(readBitmapBrightness(fPath, res),
                                                 False)))  # notepad has a light background; so the second arguement for convertImageToShades is False
            frame.close()
    ...
    def videoThread():  # thread for rendering video
        ...
        for i in range(frameCount):  # for every frame
            ...
            timePassed = t.time() - start
            if abs((timePassed * frameRate) - i) > 5: i = round(
                timePassed * frameRate);corrections += 1  # correct the frame timings
            ...
            for j in range(frameCount):  # loop for the number of frames
                ...
                if len(list(str(i + 1))) < 5:  # the number is always 5 digits long, so
                    for l in range(5 - len(list(
                            str(i + 1)))):  # we need to add zero's to the start of it in case i < 5 digits long
                        iNum.insert(0, '0')
                        iNumL.insert(0, '0')

            current = subprocess.Popen(
                ["notepad", "notepadFrames/notepadFrame" + str(''.join(iNum)) + ".txt"])  # display the frame
